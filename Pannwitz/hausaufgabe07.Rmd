...
```{r}
for (s in c(3, 4, 5)) {
    exellent <- pnorm(13, mean = mu, sd = s,lower.tail=FALSE)
    output <- paste("Bei einer Standabweichung von", s, "haben",exellent*100,"% exellent.")
    print(output)
}
```
...
Je niedriger die Standardabweichung, desto weniger sehr gute Noten gibt es. Gleichzeitig ist die Durchfallrate bei niedriger Standardabweichung sehr gering. Wenn eine hohe Standardabweichung festgelegt wird, gibt es mehr sehr gute Noten und mehr "Durchfaller".

Warum?

Einerseits ist die Range für exellente Noten um zwei Notenpunkte kleiner, 
...
Die Notenverteilung ist linksschief, da sich die höheren Notenpunkte weiter rechts auf der x-Achse stehen.
...
4. Wie viele Studenten bekommen weniger als 8 NP?
    ...
    `r (pnorm(8,mean=mu,sd=3) - pnorm(0,mean=mu,sd=3) ) * n` Studenten bekommen weniger als 8 Notenpunkte.
...
Manchmal will man in die andere Richtung gehen -- z.B. um die Frage beantworten zu können, welche Note man erreichen muss, um überdurchschnittlich zu sein. Dafür haben wir `qnorm()`. Überdurchschnittlich heißt "besser als die Hälfte abschneiden" (duh!) und wir nehmen wieder an, dass die Standardabweichung gleich 3 ist. Dann haben wir die Aussage:
...
Um unter den besten 1% abzuschließen, muss man zumindest `r qnorm(0.99,mean=mu,sd=3)` Notenpunkte bekommen.
